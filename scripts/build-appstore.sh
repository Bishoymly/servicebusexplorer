...
# Sign the app bundle with --deep to sign all nested code
# CRITICAL: The nested dependencies are already signed above, but --deep will re-sign them
# This is OK as long as we use the same identity and identifier
# CRITICAL: Use --timestamp=none to avoid hanging
# Note: TeamIdentifier should be automatically extracted from certificate UID
# 
# For App Store, we need to ensure the designated requirement is correct
# The requirement format: identifier "bundle.id" and certificate root = H"certificate_hash"
# Disable set -e temporarily to capture errors properly
set +e

# Get the certificate hash in uppercase (as required by codesign)
CERT_HASH_UPPER=$(security find-certificate -c "$SIGNING_IDENTITY" -a -Z 2>/dev/null | \
    grep "SHA-1 hash:" | awk '{print $3}' | tr '[:lower:]' '[:upper:]' || echo "")

# Use --deep to sign all nested code with the same identity
# This ensures everything has consistent signatures and requirements
# 
# CRITICAL: The "does not satisfy its designated Requirement" error occurs when
# the signature doesn't match what the requirement expects. This typically happens
# when TeamIdentifier is not extracted from the certificate.
#
# The designated requirement is automatically generated by codesign. It should be:
# identifier "com.bishoylabib.servicebusexplorer" and certificate root = H"cert_hash"
#
# Try unlocking the keychain first to ensure certificate access
# This helps codesign access the certificate and extract TeamIdentifier
security unlock-keychain ~/Library/Keychains/login.keychain-db 2>/dev/null || true

# Sign with --deep to sign all nested code
# The keychain unlock above should help codesign access the certificate
# and extract TeamIdentifier, which is required for the designated requirement
